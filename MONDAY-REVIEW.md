...
> **Historical review — not current authority.** This file preserves the September 4–5, 2026 event-readiness review and its recommendations. Use `docs/` and `README.md` for current behavior, commands, and decisions. Historical claims below may be superseded by later code/tests.

September 5 update: the first two implementation steps were reported complete. The original findings below are preserved as the pre-change review. The old defect probe script was reported as converted into regression tests. Cloudflare staging/deployment and the room-wide rehearsal were still pending at the time of this review.
...
I reviewed the conversation "Define BranDuel requirements" (01a006af-4c50-79a1-a5a1-f5543d67fbbe), inspected the current source, reran the then-existing tests and production build, and reproduced several defects against an isolated in-memory database. The reproductions use the actual Worker handler with a small SQLite adapter; they do not establish behavior under real Cloudflare concurrency. No deployment or fresh browser/device rehearsal was performed. Earlier chat claims about visual QA are historical evidence.

The implementation files were left unchanged at the time. The probe script mentioned in the original handoff was outside this repository and is not a current artifact. The review's reproduction claims should be treated as historical evidence only; current behavior is defined by code and tests.
...
|---|---|---|
| P0 | The API accepted any string action type. A non-Jimmy request using internal `HYDRATE_REMOTE` could replace shared state. This was historical evidence from the pre-change review; current validation is in [worker/index.ts](worker/index.ts) and [src/domain.ts](src/domain.ts). | Allowlist external actions, validate every payload, and keep client hydration outside the server command reducer. Reject unknown/internal actions before any write. |
...
| P0 | State, audit additions, and request completion committed separately in the pre-change review. | Commit the state, ledger entries, and request result atomically. Preserve safe revision conflict handling across the whole commit. Bind request keys to actor and payload; reuse keys on uncertain retries. A retry must return the original result without another financial change. |
| P1 | The pre-change client dispatch applied actions locally and announced success immediately. Current request handling is documented in [src/store.tsx](src/store.tsx) and [src/transport.ts](src/transport.ts). | Show pending/accepted/rejected states. Only announce accepted wagers and settled rounds after confirmation. On connection loss, retain a visibly stale read-only view and pause writes. Resolve uncertain requests by their original IDs. |
| P1 | The pre-change FFA validation checked length and membership without uniqueness. Current result validation is in [src/domain.ts](src/domain.ts). | Require an exact permutation of the participants. Validate distinct teams, participant counts, game IDs, finite safe integer money, limits, and legal lifecycle transitions on the server. Reject invalid actions explicitly instead of returning success with unchanged state. |
| P1 | The pre-change review found an indefinite reopened betting window. Current deadline behavior is in [src/engine.ts](src/engine.ts) and [src/domain.ts](src/domain.ts). | Reopening must set a new finite deadline. Use server time to accept bets and synchronize the displayed countdown. Starting play must close all access to that round's market. |
...
| P1 | The pre-change review found host controls shown to all players and an unfrozen end-of-night state. Current frontend paths are in [src/App.tsx](src/App.tsx). | Render controls by capability. Freeze and archive the final event summary, including starting/ending balances, results, and rules. Remove reset from the live event workflow. |
