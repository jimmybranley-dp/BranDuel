...
export function EconomySeasonPanel({ notify }: { notify: (message: string) => void }) {
  const { state, dispatch } = useStore();
  const canStart = state.gameNight?.status !== "active" && state.events.every((event) => ["completed", "cancelled"].includes(event.status));
  async function startSeason() {
    if (!window.confirm("Reset wallets, game history, ratings, wagers, and closeout archives for a fresh season? This cannot be undone from the app.")) return;
    const reason = window.prompt("Reason for resetting the league for a new season:");
    if (reason?.trim() && await dispatch({ type: "RESET_SEASON", reason })) notify("New season started with $200,000 per team and clean ratings.");
  }
  return <div className="admin-block economy-season-panel"><h3>Reset for a new season</h3><p className="field-note">Clear prior games, ratings, wagers, and closeout archives from the active league view, then start all four wallets at $200,000. The server keeps immutable audit records.</p><button className="secondary-button" disabled={!canStart} onClick={() => void startSeason()}>Reset and start new season</button></div>;
}
...
export function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  return <section className="admin-panel">
    ...
    <EconomySeasonPanel notify={notify} />
